...
class MNISTCNNImproved(nn.Module):
...
def build_dataloaders(
    ...
    # 验证集和测试集不要做增强
    full_train_dataset_for_val = torchvision.datasets.MNIST(
    ...
    val_dataset = torch.utils.data.Subset(full_train_dataset_for_val, val_indices)
...
def plot_error_distribution(targets, preds, save_dir, num_classes=10):
    ...
    for bar, v in zip(bars, counts):
        plt.text(bar.get_x() + bar.get_width() / 2, v + 0.1, str(int(v)), ha='center')
    plt.tight_layout()
...
def main():
    ...
    result_dir = './results_mnist_optimized'
    model_save_path = './best_mnist_cnn_optimized.pth'
    ...
    # 模型
    model = MNISTCNNImproved(num_classes=num_classes, dropout=dropout).to(device)
