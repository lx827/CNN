...
def get_channel_diagnosis(
    ...
    batch_index: int,
    channel: int,
    denoise_method: Optional[str] = Query(default=None, description="去噪方法过滤: none/wavelet/vmd/wavelet_vmd/wavelet_lms/emd/ceemdan/savgol/wavelet_packet/ceemdan_wp/eemd"),
    db: Session = Depends(get_db)
):
    """
    查询指定批次通道的诊断结果。
