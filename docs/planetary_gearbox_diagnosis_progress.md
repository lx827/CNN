...
| **v5** | — | `_gear_confidence` 行星箱独立阈值；`planetary_fullband_env_kurt` 指标；健康度扣分逻辑 | 二分类 43.75% → 55%，五类 17.5% |
| **v6** | — | `gear_gate` 保底 0.5；FM4/FM0 扣分路径；FM4 阈值 4.0→3.2；子类型推断 `_infer_gear_subtype_from_indicators`；D-S 融合缺齿类型；`label_mapper` fallback 修复 | **二分类 55% → 70%，五类 17.5% → 48.75%** |
