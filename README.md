...
>
> **默认参数：** 25600 Hz 采样率，10 秒采集时长，支持数据压缩上传。
...
边端会每 **10 秒**（可通过前端 Settings 页面动态调整，支持秒/分钟/小时）自动为所有配置的设备采集一批风机振动数据（**25600 Hz x 10 秒 = 256000 点/通道**）上传到云端，同时每 **5 秒** 轮询所有设备是否有手动采集任务。
...
| **Alarm** | 告警统计卡片、告警记录分页列表（支持级别过滤）、详情查看对话框（含通道信息、处理建议）、标记处理功能 |
| **Settings** | 边端采集配置 + **通道级振动特征告警阈值配置**（RMS/峰值/峭度/峰值因子的预警/严重阈值） |
| **新增** | 边端离线检测（5分钟无数据自动标记离线）、诊断结果关联批次（一键跳转原始数据）、WebSocket 实时推送（断线自动重连）、数据导出 CSV、批量删除批次 |
| **DataView** | 表格展示设备及所有批次，点击批次默认加载时域波形；FFT、STFT、包络谱、**阶次谱（阶次跟踪）**、**倒谱分析（Cepstrum）**、**统计指标**（峰值/RMS/峭度/偏度/裕度/波形因子/脉冲因子/峰值因子等，加窗统计量输出时序图，窗口大小和滑动步长可调）需点击按钮后按需计算，防止过度消耗算力，支持删除单条或批量特殊数据 |
...
```
1. 边端采集
   edge_client.py 每 10 秒生成振动信号（25600 Hz x 10 s = 256000 点）
...
**Q6: 如何关闭数据压缩？**
- 编辑 `edge/.env`，将 `COMPRESSION_ENABLED=false`
- 重启边端，此时上传原始 JSON，方便抓包调试
...
| 阶段 | 内容 |
|------|------|
| **当前** | 简化规则算法 + 5 设备模拟数据，系统完整可运行，支持数据压缩 |
