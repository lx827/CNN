...
按前端实际调用方式：每种轴承方法独立调用 analyze_bearing()，传正确机械参数。
- 11 种轴承方法（每种独立评估二分类：健康 vs 故障）
- Ensemble 集成（五类分类 + 二分类，基于 D-S 融合）

运行方式：
    d:\code\CNN\cloud\venv\Scripts\python.exe tests\diagnosis\method_eval\test_bearing_hustbear.py
"""
import sys
import time
import warnings
...
def run_single_method_binary(
    ...
    前端调用方式：
        ...
        result = engine.analyze_bearing(signal, fs, rot_freq=rf)
    ...
    for filepath, info in files:
        try:
            ...
            # 获取 health_score（analyze_bearing 不返回 health_score，需要单独算）
            # 实际前端用的是 /analyze 接口，返回的是 analyze_research_ensemble 结果
            # 但这里我们用二分类简单判定：根据 fault_indicators 是否有显著项
            ...
            # 如果 fault_indicators 有显著物理参数项 → fault
            pred_binary = "fault" if has_fault else "healthy"
