...
```
风机边端数据采集客户端
设备列表: WTG-001, WTG-002, WTG-003, WTG-004, WTG-005
...
|------|------|
| 统计卡片 | 严重告警数、预警数、已处理数 |
| 告警列表 | 分页表格，展示所有历史告警（支持按级别/设备过滤） |
| 刷新按钮 | 右上角重新拉取最新告警 |
| 查看详情 | 点击"查看详情"弹出详细信息对话框（含当前值、阈值对比、超标百分比） |
| 关联数据 | 点击"查看数据"跳转到 DataView 查看产生该告警的原始数据批次 |
| 标记处理 | 点击"标记处理"将告警状态改为已处理 |
| 删除 | 点击"删除"移除单条告警记录 |
...
|------|------|
| 设备选择 | 下拉选择要配置的设备 |
| 自动采集间隔 | 边端自动上传数据的时间间隔（支持秒/分钟/小时） |
| 轮询间隔 | 边端查询手动采集任务的频率（3~60 秒） |
| 采样率 | 信号采集采样率（Hz） |
| 采集时长 | 每次采集的窗口时长（秒） |
| 通道数 | 采集通道数量（1~8） |
