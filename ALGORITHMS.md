...
1. [滚动轴承故障诊断](#一滚动轴承故障诊断)
   ...
   1.7 [最大相关峭度解卷积 (MCKD)](#17-最大相关峭度解卷积-mckd)

   1.8 [时域同步平均 (TSA)](#18-时域同步平均-tsa)

   1.9 [轴承故障特征匹配与判别准则](#19-轴承故障特征匹配与判别准则)
...
#### 2.7.8 窄带包络峭度 (env_kurt) 的区分力与局限性

> **结论**：`env_kurt` 仅对磨损和缺齿敏感，对断齿和裂纹几乎无区分力——这在理论上是完全正常的。

**各故障类型的物理特征**：

| 故障类型 | 物理机制 | 预期 env_kurt | 实测数据 (20Hz) | 说明 |
|---------|---------|-------------|---------------|------|
| **健康** | 正常啮合调制 | 低（0~2） | 0.22, **2.20** | 2.2 是载波调制偏高的正常波动 |
| **磨损** | 大面积接触面粗糙 → 连续冲击 | **很高**（5~10） | 4.86, 8.02 | ✅ 最强区分特征 |
| **缺齿** | 周期性空转 → 规律性冲击 | **高**（3~6） | 3.32, 5.87 | ✅ 很好区分 |
| **断齿** | 局部断裂 → 非周期性冲击 | 中等（1.5~3） | 1.20, 2.15 | ⚠️ 与健康重叠 |
| **裂纹** | 早期微小损伤 → 微弱冲击 | 低（1~2） | 1.02, 1.34 | ❌ 几乎与健康一样 |

**为什么断齿和裂纹的 env_kurt 不高？**

- **断齿（Br）**：断齿是局部损伤，只影响一个行星轮上的少数齿。行星齿轮箱有4个行星轮同时啮合，一个行星轮上的断齿冲击被其他3个健康齿轮的啮合**平均化**了，包络峭度提升不明显。
- **裂纹（Rc）**：裂纹是最早期的损伤，裂纹长度远小于齿宽，产生的冲击能量极弱，在噪声和正常啮合调制面前被淹没。

**Samuel & Pines (2005)** 和 **Lei et al. (2014)** 推荐的指标分工：

| 故障类型 | 最敏感指标 | 原因 |
|---------|-----------|------|
| **磨损** | FM0 + env_kurt | 大面积损伤 → 时域整体波动增大 + 包络冲击增多 |
| **缺齿** | FM4 + SER | 周期性局部冲击 → 差分信号峭度高 + 边频带丰富 |
| **断齿** | FM4 + TSA残差峭度 | 断齿产生强烈的周期性冲击 → 差分信号最敏感 |
| **裂纹** | CAR + 早期SER变化 | 裂纹产生周期性微冲击 → 倒频谱能捕捉微弱周期性 |

**本项目实现决策**：

- `env_kurt` 的 `warning` 阈值设为 **3.0**（原 2.0）：仅保留对磨损和缺齿的灵敏度
- 断齿和裂纹的检测交给 `FM4` / `SER` / `CAR` / `TSA残差峭度` 等其他指标
- `env_kurt` 在 `evaluate_planetary_demod_results` 中从"核心指标"降级为"辅助指标"
- `sun_modulation_depth` 不可用做门控（健康/故障间完全重叠，35~57）

---

### 2.8 小波包能量熵 (Wavelet Packet Energy Entropy)

#### 2.8.1 原理

小波包分解对信号进行完整的二叉树分解，同时分解近似子空间（低频）和细节子空间（高频），第 $j$ 层产生 $2^j$ 个节点，覆盖整个频带。

与经典小波去噪只分解近似系数不同，小波包能量熵关注**全频带能量分布**的变化：

- **健康齿轮**：能量集中在少数几个频带（低熵）
- **故障齿轮**：局部损伤引起频带能量重分布，能量分散（高熵）

#### 2.8.2 计算公式

**节点能量**：
$$E_{j,k} = \sum_{n} |d_{j,k}[n]|^2$$

**能量概率**：
$$p_{j,k} = \frac{E_{j,k}}{\sum_{m} E_{j,m}}$$

**Shannon 能量熵**：
$$H_{wp} = -\sum_{k=0}^{2^j-1} p_{j,k} \log_2 p_{j,k}$$

**归一化熵**（便于跨信号比较）：
$$H_{norm} = \frac{H_{wp}}{\log_2(2^j)}$$

归一化熵范围 $[0, 1]$：$0$ 表示能量完全集中，$1$ 表示能量完全均匀分布。

#### 2.8.3 齿轮故障判据

| 工况 | 归一化熵范围 | 物理意义 |
|------|------------|---------|
| 健康 | $0.30\sim0.55$ | 能量集中在啮合频率及其谐波 |
| 早期磨损 | $0.55\sim0.70$ | 边频带能量增加，分布开始分散 |
| 局部损伤 | $>0.70$ | 冲击成分使高频能量显著增加 |

#### 2.8.4 工程参数建议

| 参数 | 推荐值 | 说明 |
|------|--------|------|
| 小波基 | `db8` / `sym8` | 与现有小波去噪保持一致 |
| 分解层数 $j$ | 3~4 | 第3层→8节点(各512Hz@8192Hz)；第4层→16节点(各256Hz) |
| 啮合频带集中度 | $>0.15$ | 健康齿轮啮合频带能量占比 |

#### 2.8.5 与现有齿轮指标的互补性

- **SER/CAR**：关注啮合频率附近的边频带结构
- **FM4**：关注差分信号的局部峭度
- **小波包能量熵**：关注**全频带能量分布**，对高频冲击引起的能量重分布敏感
- **推荐组合**：SER + FM4 + 能量熵，覆盖粗故障→局部损伤→频带重分布

**参考实现**：`cloud/app/services/diagnosis/wavelet_packet.py`
...
#### 4.2.5 Savitzky-Golay 多项式平滑（计算极快、保留峰形）

**核心原理**：

对信号中每个点，取其邻域窗口 $[-w, w]$ 内的 $2w+1$ 个点，用 $p$ 阶多项式最小二乘拟合：

$$x_{smooth}[n] = \sum_{m=-w}^{w} c_m \cdot x[n+m]$$

其中 $c_m$ 为仅与窗口长度和多项式阶数有关的固定卷积系数（通过伪逆求解 Vandermonde 矩阵得到）。

**独特优势**：

- 在平滑噪声的同时，**较好保留信号的峰宽、峰值高度和面积**
- 不像滑动平均那样模糊尖锐特征
- 计算复杂度 $O(N)$，极快

**工程参数选取**：

| 应用场景 | 窗口长度 | 多项式阶数 | 物理意义 |
|---------|---------|-----------|---------|
| 轴承诊断预处理 | 51~101 | 3~5 | 覆盖 6~12 ms，平滑高频噪声 |
| 齿轮诊断预处理 | 101~201 | 3~4 | 覆盖 12~25 ms，保留啮合冲击 |
| 慢趋势提取 | 501~1001 | 2~3 | 覆盖 60~120 ms，提取慢漂移 |

**与小波阈值去噪的互补性**：

- 小波阈值：适合脉冲型/非平稳噪声，但可能产生伪吉布斯振荡
- S-G 滤波：适合宽带高斯噪声背景下的平滑，保留峰形

**参考实现**：`cloud/app/services/diagnosis/savgol_denoise.py`

#### 4.2.6 CEEMDAN 完备集成经验模态分解（抗模态混叠）

**背景**：

- **EMD**（经验模态分解）：基于时域局部特征尺度自适应提取 IMF，无需预设模态数，但存在模态混叠问题
- **EEMD**（集成 EMD）：加入噪声辅助，但引入重构误差和残留噪声
- **CEEMDAN**（完备 EEMD）：在每一阶段用残余的局部标准差自适应调整噪声幅值，无直流残留，重构误差为零

**CEEMDAN 算法流程**：

1. 对 $M$ 次实现加入自适应白噪声：$x^{(i)}(t) = x(t) + \beta_0 E_1(w^{(i)}(t))$
2. 分别做 EMD，平均得到 IMF1
3. 计算残余 $r_1 = x - \text{IMF1}$
4. 对 $r_1 + \beta_1 E_1(\text{noise})$ 做 EMD 提取 IMF2
5. 依此类推，噪声幅值逐层自适应调整

**IMF 筛选与重构（降噪策略）**：

$$x_{denoised} = \sum_{i \in \mathcal{S}} c_i(t), \quad \mathcal{S} = \{i \mid \rho(c_i, x) > 0.35 \lor \text{Kurt}[c_i] > 3.5\}$$

- 跳过 IMF1（高频噪声）和最后一阶 IMF（低频趋势）
- 保留高相关或高峭度的中间层 IMF
...
| 优化方式 | 频域变分优化 | 时域局部尺度 |
| 边界效应 | 较小 | 需镜像处理 |
| 计算速度 | 中等 | 较慢（$O(M \times N^2)$）|
| 适用工况 | 共振频带明确 | 转速剧烈波动 |
| 推荐 profile | balanced / exhaustive | exhaustive 专用 |

**参考实现**：`cloud/app/services/diagnosis/emd_denoise.py`
