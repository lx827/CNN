...
> **诊断算法体系文档**：`docs/DIAGNOSIS_SYSTEM_OVERVIEW.md`（5 子系统串联，方法公式/投票/评分） + `ALGORITHMS.md`（经典理论推导，故障频率/TSA/谱峭度等）
...
| 文档路径 | 用途 |
|----------|------|
| `docs/backend/app/INDEX.md` | **后端代码目录索引** — 每个 `.py` 文件对应一个 `.md`，严格按源码目录结构组织，包含函数签名、参数、返回值、说明 |
...
- **新增 / 修改 / 删除 API 端点时** → 同步更新 `docs/api/backend_api.md` 和 `docs/api/frontend_backend_interaction.md`，并在对应路由 `.md` 文件中更新端点说明
- **新增 / 修改 / 删除服务层公共函数时** → 同步更新 `docs/backend/app/services/` 下对应模块的 `.md` 文件
- **新增 / 修改 / 删除前端 API 封装或 Vue 组件时** → 同步更新 `docs/frontend/frontend_api.md` 和 `docs/api/frontend_backend_interaction.md`
- **新增 / 删除 / 重命名 `.py` 文件时** → 在 `docs/backend/app/` 下同步创建 / 删除 / 重命名对应 `.md` 文件，并更新 `INDEX.md`
- **修改函数签名（参数、返回值类型）时** → 同步更新对应 `.md` 文件
...
- 修改 `cloud/app/api/` 下的路由/端点前 → 查阅 `docs/api/backend_api.md` + `docs/backend/app/api/` 下对应 `.md`
- 修改 `cloud/app/services/` 下的函数/类前 → 查阅 `docs/backend/app/services/` 下对应模块 `.md`
...
> **Bug 注册表**: `docs/BUG_REGISTRY.md` — 所有已发现 Bug 的汇总，含已修复和未修复。
